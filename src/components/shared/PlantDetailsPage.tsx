    ...
    <div className="min-h-screen bg-[#FAFAF8] dark:bg-slate-950 py-8 sm:py-12 transition-colors duration-200">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <nav className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="text-slate-800 dark:text-slate-100 font-semibold truncate max-w-[200px]">{plant.title}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
          ...
          <div className="lg:col-span-6 space-y-4">
            {/* Main Large Image */}
            <div className="relative aspect-square w-full rounded-3xl overflow-hidden bg-slate-100 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm group">
              <img
          ...
          <div className="lg:col-span-6 flex flex-col space-y-6">
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-2xl sm:text-4xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
                {plant.title}
              </h1>
            ...
            {/* Price Box */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-between transition-colors">
              ...
              <div>
                {plant.quantity > 0 ? (
                  <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100/80 dark:bg-emerald-950/60 px-3 py-1.5 rounded-full">
                    <CheckCircle2 size={14} />
                    In Stock ({plant.quantity})
                  </span>
                ) : (
                  <span className="text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-100 dark:bg-rose-950/60 px-3 py-1.5 rounded-full">
                    Out of Stock
                  </span>
                )}
              </div>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-4">
                {/* Stepper */}
                <div className="flex items-center border border-slate-200 dark:border-slate-700 rounded-2xl p-1 bg-white dark:bg-slate-800">
                  <button
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    disabled={quantity <= 1 || plant.quantity === 0}
                    className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40"
                  >
                    <Minus size={16} />
                  </button>
                  <span className="w-12 text-center font-bold text-sm text-slate-800 dark:text-slate-100">
                    {quantity}
                  </span>
                  <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 font-medium">
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-transparent dark:border-slate-800">
        ...
        <div className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm p-6 sm:p-10 space-y-6 transition-colors">
          {/* Tab Buttons */}
          ...
          {activeTab === "care" && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
              <div className="p-4 rounded-2xl bg-emerald-50/60 dark:bg-slate-800/80 border border-emerald-100 dark:border-slate-700 space-y-2">
                <h4 className="font-bold text-emerald-900 dark:text-emerald-400">☀️ Sunlight</h4>
                <p className="text-slate-600 dark:text-slate-300">Thrives best in moderate to bright indirect sunlight. Avoid scorching direct midday heat.</p>
              </div>
              <div className="p-4 rounded-2xl bg-sky-50/60 dark:bg-slate-800/80 border border-sky-100 dark:border-slate-700 space-y-2">
                <h4 className="font-bold text-sky-900 dark:text-sky-400">💧 Watering</h4>
                <p className="text-slate-600 dark:text-slate-300">Water once every 7–10 days when the top 2 inches of soil feel dry to the touch.</p>
              </div>
              <div className="p-4 rounded-2xl bg-amber-50/60 dark:bg-slate-800/80 border border-amber-100 dark:border-slate-700 space-y-2">
                <h4 className="font-bold text-amber-900 dark:text-amber-400">🌱 Soil & Fertilizer</h4>
                <p className="text-slate-600 dark:text-slate-300">Use well-draining organic potting mix. Feed monthly with balanced organic liquid fertilizer.</p>
              </div>
            </div>
          )}
