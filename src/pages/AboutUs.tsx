      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 max-w-5xl mx-auto">
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-xs text-center space-y-1 transition-colors"
            >
              <p className="text-2xl sm:text-4xl font-extrabold text-[#81ba00]">{stat.number}</p>
              <p className="text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {values.map((v, idx) => (
            <div
              key={idx}
              className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-xs space-y-4 hover:shadow-md transition-all"
            >
              <div className="w-12 h-12 rounded-2xl bg-[#81ba00]/10 text-[#81ba00] flex items-center justify-center">
                {v.icon}
              </div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{v.title}</h3>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{v.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center space-y-8 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {team.map((member, idx) => (
              <div
                key={idx}
                className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-xs text-center space-y-3 hover:border-[#81ba00]/40 transition-all"
              >
                <div className="w-16 h-16 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-2xl flex items-center justify-center mx-auto shadow-inner">
                  {member.emoji}
                </div>
                <div>
                  <h3 className="text-sm sm:text-base font-bold text-slate-800 dark:text-slate-100">{member.name}</h3>
